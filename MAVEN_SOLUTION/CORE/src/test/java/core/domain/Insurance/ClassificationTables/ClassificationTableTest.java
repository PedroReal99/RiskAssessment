...
public class ClassificationTableTest {
    ...
    /**
     * Test of obtainMatrix method, of class ClassificationTable.
     */
    @Test
    public void testObtainMatrix() {
        System.out.println("obtainMatrix");
        List<TableLine> expResult = list;
        List<TableLine> result = t.obtainMatrix();
    ...
    public int hashCodeCalculator() {
        ...
        hash = 83 * hash + (int) (t.getId() ^ (t.getId() >>> 32));
        return hash;
    }
    
    @Test
    public void testHashCode() {
        assertEquals(t.hashCode(), hashCodeCalculator());
    }

    /**
     * Test of sameAs method, of class ClassificationTable.
     */
    @Test
    public void testSameAs() {
        Object obj = new ClassificationTable(list);
        assertTrue(t.sameAs(t));
        assertFalse(t.sameAs(null));
        assertTrue(t.sameAs(obj));
        obj = new ClassificationTable(list2);
        assertFalse(t.sameAs(obj));
        obj = new Object();
        assertFalse(t.sameAs(obj));
    }

    /**
     * Test of identity method, of class ClassificationTable.
     */
    @Test
    public void testIdentity() {
        System.out.println("identity");
        long expResult = 0;
        long result = t.identity();
        assertEquals(expResult, result);
    }

    /**
     * Test of calculateFinalScore method, of class ClassificationTable.
     */
    @Test
    public void testCalculateFinalScore() {
        System.out.println("calculateFinalScore");
        RiskMatrix rm = createMatrixTest();
        CalculationDetail dets = new CalculationDetail(new ArrayList<>());
        boolean det = false;
        int expResult = 100;
        ...
        list.add("Hospital Sao Joao, Porto");
        for(TableLine tl:t1.obtainMatrix()){
            Map<RiskFactor,ClassificationColumn> map=tl.obtainColumns();
            for(RiskFactor rf:map.keySet()){
                map.get(rf).setRelevantSurroundings(list);
            }
        }
        int result = t1.calculateFinalScore("ISEP,Porto",rm, dets, det);
        assertEquals(expResult, result);
    }

    /**
     * Test of calculateObtainedScore method, of class ClassificationTable.
     */
    @Test
    public void testCalculateObtainedScore() {
        System.out.println("calculateObtainedScore");
         RiskMatrix rm = createMatrixTest();
        CalculationDetail dets = new CalculationDetail(new ArrayList<>());
        int expResult = 94;
        Map<RiskFactor,ClassificationColumn> map1test = new HashMap<>();  
        ...
        int result = t1.calculateObtainedScore("ISEP,Porto",rm, dets);
        assertEquals(expResult, result);
    }

    /**
     * Test of calculateMaxScore method, of class ClassificationTable.
     */
    @Test
    ...
    public CaseI createCaseTest() {
        ...
        li.add(ioteste);
        li.add(io2teste);
        CaseI c = new CaseI(ct1, cs1, ccc1, li,false, CasePriority.MAX,da);
        return c;
    }
